...
class SIGPlusEngineServices {
	...
	/** True if the engine is used on a site that is publicly available (i.e. not an intranet network). */
	public $ajaxapi = true;
	/** Safe mode for external library inclusion. */
	...
	*/
	public function addJQuery() {
		...
		if ($this->ajaxapi) {  // use jQuery from Google AJAX library
			if ($this->safemode) {
				...
				if ($this->debug) {
					$document->addScript('http://ajax.googleapis.com/ajax/libs/jquery/1.4.2/jquery.js');  // duplicates are ignored
				} else {
					$document->addScript('http://ajax.googleapis.com/ajax/libs/jquery/1.4.2/jquery.min.js');
				}
			}
		} else {  // use local copy of jQuery
			$document->addScript(JURI::base(true).'/plugins/content/sigplus/js/jquery.js');
		}
		$document->addScriptDeclaration('if (typeof(__jQuery__) == "undefined") { var __jQuery__ = jQuery.noConflict(); }');
		$this->jquery = true;
	}
...
class SIGPlusLightboxEngine extends SIGPlusEngine {
	/**
	* Default constructor that ignores all optional parameters.
	*/
