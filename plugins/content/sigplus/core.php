...
class SIGPlusCoreConfiguration {
	...
	public function validate() {
		$this->ajaxapi = (bool) $this->ajaxapi;
...
class SIGPlusCore {
	...
	/**
	* Creates a thumbnail image and a preview image for an original.
	...
	*/
	private function createPreviewImage($imagedirectory, $imagefile) {
		...
		$result = create_thumbnail($this->imageservices->getLibrary(), $imagepath, $previewpath, $params->width, $params->height, $params->crop, $params->quality);
		if (SIGPLUS_LOGGING) {
			$logging =& SIGPlusLogging::instance();
			if ($result) {
		...
		if ($thumbpath != $previewpath) {  // separate thumbnail image is required
			$thumbparams = $this->imageservices->getThumbnailParameters($params);
			$result = create_thumbnail($this->imageservices->getLibrary(), $imagepath, $thumbpath, $thumbparams->width, $thumbparams->height, $thumbparams->crop, $thumbparams->quality);
			if (SIGPLUS_LOGGING) {
				$logging =& SIGPlusLogging::instance();
				if ($result) {
	...
	/**
	* Ensures that a thumbnail image exists for an original image.
	*/
	private function checkPreviewImage($imagedirectory, $imagefile) {
		$params = new SIGPlusPreviewParameters($this->curparams);
		$previewpath = $this->imageservices->getPreviewPath($imagedirectory, $imagefile, $params);
		if ($previewpath !== false && !is_file($previewpath)) {  /* create thumbnail on-the-fly if not exists */
			return $this->createPreviewImage($imagedirectory, $imagefile);
		}
		$thumbpath = $this->imageservices->getThumbnailPath($imagedirectory, $imagefile, $params);
		if ($thumbpath !== false && $thumbpath != $previewpath && !is_file($thumbpath)) {  /* create thumbnail on-the-fly if not exists */
			return $this->createPreviewImage($imagedirectory, $imagefile);
		}
		return true;
	}
	...
	*/
	private function getUserDefinedImageGalleryHtml($imagedirectory, $imagefolder, $labels, $galleryid, &$count) {
		...
		foreach ($labels as $label) {
			$this->checkPreviewImage($imagedirectory, $label->imagefile);
			$count++;
		}
	...
	*/
	private function getLabeledImageGalleryHtml($imagedirectory, $imagefolder, $files, $labels, $galleryid, &$count) {
		...
		foreach ($files as $file) {
			$this->checkPreviewImage($imagedirectory, $file);
			$count++;
		}
		switch ($this->curparams->linkage) {
			case 'inline':
