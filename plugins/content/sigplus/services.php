...
class SIGPlusImageLabel {
	...
	function __construct($imagefile, $caption, $description) {
...
class SIGPlusImageServices {
	...
	*/
	private function createDirectoryOnDemand($directory) {
		if (!is_dir($directory)) {  // directory does not exist
			@mkdir($directory, 0777, true);  // try to create it
			if (!is_dir($directory)) {
	...
	*/
	public function getThumbnailParameters(SIGPlusPreviewParameters $params) {
		if ($params->width > 100 || $params->height > 100) {
			$ratio = ($params->width >= $params->height ? $params->width : $params->height) / 100;
			$thumbparams = clone $params;
	...
	*/
	public function getThumbnailPath($imagedirectory, $imagefile, SIGPlusPreviewParameters $params) {
		return $this->getGeneratedImagePath($this->config->thumbsfolder, $imagedirectory, $imagefile, $this->getThumbnailParameters($params));
	}
	...
	*/
	public function getThumbnailUrl($imagefolder, $imagefile, SIGPlusPreviewParameters $params) {
		return $this->getGeneratedImageUrl($this->config->thumbsfolder, $imagefolder, $imagefile, $this->getThumbnailParameters($params));
	}
