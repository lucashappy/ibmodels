	...
	*/	
	$.fn.boxplusSlider = function (settings) {
		...
		var defaults = {
			...
			animationDelay: 0,          // time between successive automatic slide steps [ms], or 0 to use no automatic sliding
		...
		lists.each(function () {
			...
			var delay = settings.animationDelay;
			...
			listitems.each(function () {
				...
				if (isAsynchronousImage(image)) {
					$(this).one('boxplus-load', loadImageAsynchronously);
					/*image.one('click', function () {
						showImages(listitems.unbind('boxplus-load', loadImageAsynchronously));
					});*/
				}
			});
			...
			/**
			* Determines whether an image is to be loaded asynchronously.
			*/
			function isAsynchronousImage(image) {
				return /\.(gif|jpe?g|png)$/i.test(image.attr('longdesc'));
			}
			...
			function showImages(listitems) {
				listitems.each(function () {
					...
					if (isAsynchronousImage(image)) {
						...
						$(this).removeClass('boxplus-loading').children().removeClass(CLASS_DISABLED);
					}
				});
			}
