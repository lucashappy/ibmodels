...
class SIGPlusBoxPlusEngine extends SIGPlusLightboxEngine {
	...
	public function addStyles() {
		...
		$this->addCustomTag('<!--[if lt IE 8]><link rel="stylesheet" href="'.JURI::base(true).'/plugins/content/sigplus/engines/boxplus/popup/css/boxplus.'.$this->theme.'.ie7.css" type="text/css" title="boxplus-'.$this->theme.'" /><![endif]-->');
	}

	protected function addCommonScripts() {
		$this->addJQuery();
		$document =& JFactory::getDocument();
		$document->addScript(JURI::base(true).'/plugins/content/sigplus/engines/boxplus/popup/js/'.$this->getScriptFilename().'.js');
	...
	public function addScripts($galleryid) {
		...
		$script =
			'__jQuery__("#'.$galleryid.'").boxplusGallery({ '.
				'theme: "'.$this->theme.'",'.
